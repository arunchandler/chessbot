...
def terminal_value_from_root(board: chess.Board, root_player: chess.Color) -> float:
    """
    Return terminal value from the fixed root player's perspective.

    -1.0 if root_player lost, +1.0 if they won, 0.0 for draws.
    Raises ValueError if the game is not over.
    """
    if not is_terminal(board):
        raise ValueError("Board is not in a terminal state.")
    ...
    winner_color = chess.WHITE if outcome.winner else chess.BLACK
    return 1.0 if winner_color == root_player else -1.0
