"""Play a batch of games vs a material-eval MCTS baseline and report results."""
...
def play_match(bot, baseline, games: int) -> dict[str, int]:
    results = {"win": 0, "draw": 0, "loss": 0}
    for i in range(games):
        board = game.new_game()
        bot_color = chess.WHITE if i % 2 == 0 else chess.BLACK
        base_color = chess.BLACK if bot_color == chess.WHITE else chess.WHITE
        agents = {bot_color: bot, base_color: baseline}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate bot vs material MCTS baseline.")
    parser.add_argument("--games", type=int, default=10, help="Number of games to play.")
    parser.add_argument("--iterations", type=int, default=300, help="MCTS iterations for the bot.")
    parser.add_argument(
        "--baseline-iterations", type=int, default=300, help="MCTS iterations for the baseline."
    )
    ...
    baseline = MaterialMCTSAgent(iterations=args.baseline_iterations, c=args.cpuct)

    results = play_match(bot, baseline, args.games)
